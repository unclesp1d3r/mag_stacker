...
- **PRs squash-merge, so the PR title and body ARE the commit message.** This repo merges with `squash_title: PR_TITLE` and `squash_msg: PR_BODY`; every branch commit message is discarded at merge. Therefore:
  - The **PR title MUST be a valid Conventional Commit subject** — `<type>[(<scope>)][!]: <description>` — since it becomes the subject line on `main` verbatim. The scope is optional (`docs: …` is as valid as `docs(agents): …`).
  - A breaking change **MUST be marked on the PR itself**: a `!` before the colon in the PR title **and** a `BREAKING CHANGE: <what breaks>` footer in the PR body. The `!` is valid with or without a scope — `feat!: …` and `feat(scope)!: …` both work. Marking it only on a branch commit does not work — the marker is destroyed by the squash. A prose "Breaking change" heading in the body is not the footer and does not count.
