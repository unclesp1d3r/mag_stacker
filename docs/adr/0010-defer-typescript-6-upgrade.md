...
Dependabot bumps `typescript` `^5.9.3` → `^6.0.3` (a major, released 2026-03-23). Under 6.0, `bun run typecheck` (`tsc --noEmit`) fails with `Cannot find module 'bun:test'` across ~30 test files, plus downstream implicit-`any` errors in `src/domain/bulkadd/__tests__/labels.test.ts`. `main` on TS 5.9.3 typechecks clean.
...
Stay on TypeScript **5.9.x** for now. Do **not** merge the bump; the live tracker is **#43** (the `bun`-ecosystem re-proposal after ADR-context PR #42 merged — the original npm-ecosystem PR #5 was closed as superseded). Revisit the 6.0 upgrade when the Bun type packages resolve cleanly under TS 6, or when we plan the eventual move to TS 7.
...
### Alternative 2: Close the TypeScript bump PR outright
...
- The bump PR (#43) lingers open and red until revisited.
