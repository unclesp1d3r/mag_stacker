...
- **shadcn is wired but unused.** `.mcp.json` registers the `shadcn` MCP server and `shadcn@^4.21.0` is a devDependency, but there is **no `components.json`**, no generated components, and **no `@tanstack/react-table`**. `components/ui/cn.ts` is a hand-rolled `classes.filter(Boolean).join(" ")` — **not** clsx+tailwind-merge — so shadcn-generated components need `cn` reconciled. Tailwind v4 is CSS-first: no `tailwind.config.*`; tokens live in `app/globals.css` under `@theme inline` (`--paper`, `--ink`, `--blaze`, `--radius`, `--radius-lg`, …), not shadcn's default `--background`/`--foreground`.
