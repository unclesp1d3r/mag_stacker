...
> **Product Contract preservation:** Product Contract unchanged. Enrichment adds only HOW (Key Technical Decisions, Implementation Units, Verification Contract, Definition of Done). Requirement, Flow, and Acceptance-Example IDs (R1–R19, F1–F2, AE1–AE5) are preserved verbatim. Four questions the Product Contract deferred "to planning" are resolved below as KTD-4 through KTD-7 (optional-column sets, default page size, firearm group aggregate, localStorage schema + no-flash mechanism).
...
- **Requirements:** R3.
- **Dependencies:** U1.
- **Files:** `hooks/use-table-view-state.ts`, `components/ui/data-table/data-table.tsx` (consume the hook), `hooks/__tests__/` (only if any pure serialize/migrate helper is extracted; interactive behavior is e2e).
...
- **Approach:** Consumes `{ groups, borrowed }` from the U3 engine and the shared column definitions from the U1 wrapper (member rows reuse the same columns). Each group renders a header row (group name, count, magazine capacity aggregate when present per KTD-6/R12) with an accessibly-named expand/collapse control (`aria-expanded`, `aria-controls`; button is keyboard-operable, R15). Collapsed by default (R11). Expand animates height/opacity with Motion (`from "motion/react"`, `AnimatePresence`, easing `[0.16, 1, 0.3, 1]`, ~200ms) and honors `prefers-reduced-motion` with an instant fallback (R17). While grouping is active, all groups render on one page with no pagination (R14); member sort overrides default label order when a sort is set (R13, applied via the shared wrapper's sort state). Below `md`, expanded members stack vertically (R19). Empty states reuse `EmptyState` (R16): empty "Shared with you", filter-excludes-every-row, and grouping-with-zero-groups — each with a teaching message, not a bare blank.
...
- **R-D — large inventories (>500) degrade client-side ops.** *Mitigation:* documented ceiling assumption; server-side pagination is an explicit deferred follow-up, not silently absent.
