...
- **Wildcard key-denylist redaction.** Sensitive fields are redacted by key name anywhere in the logged object tree via wildcards, giving the broadest automatic coverage for structured fields. Emails are fully redacted (no partial-mask variant).
...
- R8. A curated denylist of sensitive key names — serial numbers, session tokens, passwords, emails, authorization headers, and similar — is redacted anywhere in the logged object tree via wildcard key matching.
