...
- KTD4. **`brandModel` matches by required-substring containment against an ordered list; first match wins.** Normalize by uppercasing and removing every character outside `A-Z0-9`, yielding one dense token (`"Magpul PMAG 17 GL9"` -> `MAGPULPMAG17GL9`). Each list entry carries the substrings a model must contain, keyed on the *distinctive family marker alone* (`["GL9"]` -> 2 cells) rather than on a brand token as well. Requiring `PMAG` would reject the natural shorthand `Magpul GL9`, because `MAGPUL` does not contain the substring `PMAG` — and a miss there is not harmless, since it drops an unambiguous 2-cell magazine to the unverified 4-cell fallback. Exact matching cannot work on free text, and a regex-per-entry list is harder to audit than a token set. Order the list most-specific-first so a narrow family shadows a broad one. The counts stored in the list are per-face, per the Product Contract's one-face Key Decision (Governs R3, R5). Governs R3, R4, R5.
...
3. Seed the list with the two families R5 names, keyed on the distinctive family marker only (KTD4): the GL9 family at 2 cells (`["GL9"]`), and the 7.62x51 PMAG family at 4 cells as two entries, `["LRSR"]` for Magpul's `PMAG 20 LR/SR GEN M3` naming and `["762X51"]` for the caliber spelled into the model string. Do not add a `PMAG` token to any entry — `MAGPUL` does not contain the substring `PMAG`, so requiring it would reject `Magpul GL9`. Add a comment recording that the list is expected to grow as counts are sourced, and that a wrong count is worse than a missing one because a matched entry carries no caveat.
...
- `"Magpul PMAG 25 7.62x51"` resolves to 4 cells, matched via the caliber entry, proving normalization collapses `7.62x51` to `762X51`.
