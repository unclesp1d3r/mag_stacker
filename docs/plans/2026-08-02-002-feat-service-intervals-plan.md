...
- An accessory's `installed_date` is cleared whenever the accessory is unmounted, enforced in the service layer and backed by a CHECK constraint, so it cannot serve as a stable origin date. R10 instead resolves an accessory's origin date the same way as a firearm's: `acquired_date` when set, otherwise `created_at`.
...
- KTD9. **Origin date resolves identically for both families:** `acquired_date` when set, otherwise `created_at` as a calendar date. Accessories gained their own `acquired_date` column during implementation (see Scope added during implementation) precisely so they follow this same rule rather than always falling back to `created_at` — the fallback-only version reproduces the cold-start problem KD6 exists to prevent, for any accessory owned before it was entered into MagStacker. Accessories still cannot use `installed_date` as an origin date; it is force-nulled on unmount. Cites R10, R22.
...
- **Test scenarios:**
  ...
  - An accessory with an acquired date measures from that date; one without measures from its creation date, not its installed date.
