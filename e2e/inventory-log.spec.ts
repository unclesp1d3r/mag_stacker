 ...
 * R10 ("Mark inventoried"), R11 ("Log…" form's event-type select, which now
 * offers only "inventoried" for either parent family — the service-intervals
 * plan's U5 retired `cleaned`/`lubed` as firearm event types (R13), converting
 * every prior entry to a `service_event`), and R12 (ARIA targeting, no
 * data-testid) through the real browser on one seeded "inventory-log" user.
 * Sharing/actor-attribution (R7/R8) is covered in
...
test("logs, orders newest-first, and offers only 'inventoried' for both parents (U5)", async ({
  page,
}) => {
  ...
  await test.step("a firearm's event-type select offers only 'inventoried' (R11/R13 — cleaned/lubed retired, U5)", async () => {
    await page.getByRole("button", { name: "Log…" }).click();
    const form = page.locator("form").last();
    const eventSelect = form.getByLabel("Event type");
  ...
  await test.step("Log… records backdated entries above an older one, newest-first (R9)", async () => {
    await page.getByRole("button", { name: "Log…" }).click();
    let form = page.locator("form").last();
    await form.getByLabel("Event type").selectOption("inventoried");
    await form.getByLabel("Date & time").fill("2026-01-15T09:00");
    await form.getByLabel("Notes").fill("First scheduled check");
    ...
    await form.getByLabel("Notes").fill("Second scheduled check");
    await form.getByRole("button", { name: "Log" }).click();
    await expect(page.getByText("Logged").first()).toBeVisible();

    // Newest-first (R9): the just-marked "now" entry stays on top, the more
    // recently-dated backdated entry sorts above the older one, and its notes
    // render alongside it.
    const rows = logTable(page).locator("tbody tr");
    await expect(rows).toHaveCount(3);
    await expect(rows.nth(0)).toContainText("Inventoried");
    await expect(rows.nth(1)).toContainText("Inventoried");
    await expect(rows.nth(1)).toContainText("Second scheduled check");
    ...
    await expect(rows.nth(2)).toContainText("Inventoried");
    await expect(rows.nth(2)).toContainText("First scheduled check");
  });
