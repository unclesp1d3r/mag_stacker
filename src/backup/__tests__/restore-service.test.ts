...
/**
 * Every test below drives a REAL Postgres container through key derivation, tar
 * packing, a full-database wipe, and (in two cases) advisory-lock serialization
 * of two concurrent restores. Several legitimately take 5–8 seconds, so bun's
 * 5s default is a load tripwire rather than a signal about the code.
 *
 * Tripping it is not a contained failure. Bun marks the test failed but the
 * timed-out body keeps running, so `afterAll` ends the pool underneath it and
 * every remaining test in the file cascades into "Cannot use a pool after
 * calling end on the pool" — failures that name the wrong test and point at
 * nothing real. That is what made `just ci-check` fail in a different, larger
 * subset on each run while this file passed 14/14 in isolation.
 *
 * 60s is ~7x the slowest observed run, so a genuine hang still fails, just
 * later. Prefer this over raising the suite-wide default, which would hide
 * slowness in the ~1250 tests that have no business taking seconds.
 */
const CONTAINER_TEST_TIMEOUT_MS = 60_000;

function containerTest(name: string, body: () => Promise<void>): void {
  test(name, body, CONTAINER_TEST_TIMEOUT_MS);
}
...
describe("restore service (U5)", () => {
  ...
  containerTest(
    "empty instance + valid bundle + matching version promotes; the instance ends up equal to the source (AE5/R10)",
    async () => {
  ...
  containerTest(
    "non-empty instance + plain restore is refused; nothing changes (AE1/R6)",
    async () => {
  ...
  containerTest(
    "non-empty instance + force restore wipes then promotes (AE2/R7)",
    async () => {
  ...
  containerTest(
    "wrong password is refused; live data untouched (AE3/R9)",
    async () => {
      await seedInventory(db, uploadDir);
      const beforeSnapshot = await snapshotTables(db);
      const beforeFiles = await readUploadDirKeys(uploadDir);
  ...
  containerTest(
    "a tampered byte in the LAST chunk is refused before promote — proves stage-then-promote (KTD10)",
    async () => {
  ...
  containerTest(
    "version mismatch is refused before staging (AE4/R8)",
    async () => {
      await seedInventory(db, uploadDir);
      const beforeSnapshot = await snapshotTables(db);
      const beforeFiles = await readUploadDirKeys(uploadDir);
  ...
  containerTest(
    "fault injected inside the wipe+promote transaction rolls back both DB and blobs (critical path, pre-commit)",
    async () => {
      const preExisting = await seedInventory(db, uploadDir);
      const beforeSnapshot = await snapshotTables(db);
      const beforeFiles = await readUploadDirKeys(uploadDir);
      ...
      expect(
        (
          await readFile(join(uploadDir, preExisting.documentStorageKey))
        ).equals(beforeBlobContent),
      ).toBe(true);
    },
  );

  containerTest(
    "fault injected after the DB commits but before the blob swap rolls back both stores via the snapshot schema (critical path, post-commit)",
    async () => {
      const preExisting = await seedInventory(db, uploadDir);
      const beforeSnapshot = await snapshotTables(db);
      const beforeFiles = await readUploadDirKeys(uploadDir);
  ...
  containerTest(
    "a path-traversal blob entry is refused; nothing is written outside staging (KTD11) — and surfaces as a generic thrown error, not wrong_password_or_tampered",
    async () => {
  ...
  containerTest(
    "a genuine wrong password still maps to wrong_password_or_tampered, not a generic thrown error",
    async () => {
      await seedInventory(db, uploadDir);
      const beforeSnapshot = await snapshotTables(db);
      const beforeFiles = await readUploadDirKeys(uploadDir);
  ...
  containerTest(
    "a non-admin caller is refused inside the service, independent of any route gate (R14)",
    async () => {
      const beforeSnapshot = await snapshotTables(db);
      const bundle = await buildEncryptedBundle(db, {});
  ...
  containerTest(
    "two concurrent restore() calls against the same instance serialize via the per-run advisory lock; the final state is exactly one bundle's, not a mix (KTD5 hardening)",
    async () => {
      ...
      expect(
        outcomes.filter((o) => o.kind === "refused_not_empty"),
      ).toHaveLength(1);
  ...
  containerTest(
    "the empty-instance (non-force) restore path also enters maintenance during promote, blocking concurrent ordinary writes (TOCTOU fix)",
    async () => {
  ...
  containerTest(
    "_testFaultInjection is refused outside a test environment (production footgun guard)",
    async () => {
