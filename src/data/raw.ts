// Generated from src/data/*.txt — curated reference lists embedded as a
// module so loading needs no filesystem access (works in the Next bundle,
// standalone output, and Docker). Edit the .txt files then regenerate.

export const CALIBERS_RAW = `Cartridge
.17 HMR
.17 WSM
.22 LR
.22 Magnum
.22 Short
.22-250 Remington
.223 Remington
.224 Valkyrie
.243 Winchester
.25 ACP
.25-06 Remington

Common Rifle Caliber Name
.260 Remington
.270 Weatherby Magnum
.270 Winchester
.270 WSM
.280 Ackley Improved
.280 Remington
.30 Carbine
.30 Nosler
.30-06 Springfield
.30-30 Winchester
.300 AAC Blackout
.300 H&H Magnum
.300 Norma Magnum
.300 PRC
.300 Remington Ultra Magnum
.300 Weatherby Magnum
.300 Win Mag
.300 WSM
.303 British
.308 Winchester
.32 ACP
.32 H&R Magnum
.325 WSM
.327 Federal Magnum
.33 Nosler
.338 Federal
.338 Lapua Magnum
.338 Norma Magnum
.338 Win Mag
.340 Weatherby Magnum
.35 Remington
.35 Whelen
.350 Legend
.357 Magnum
.357 SIG
.375 H&H Magnum
.375 Ruger
.38 Special
.38 Super
.380 ACP

Handgun Cartridge
.40 S&W
.400 Legend
.404 Jeffery
.41 Magnum
.416 Barrett
.416 Remington Magnum
.416 Rigby
.44 Magnum
.44 Special
.444 Marlin
.45 ACP
.45 Colt
.45-70 Government
.450 Bushmaster
.450 Marlin
.454 Casull
.458 SOCOM
.458 Winchester Magnum
.460 S&W Magnum
.460 Weatherby Magnum
.480 Ruger
.50 Action Express
.50 Beowulf
.50 BMG
.500 S&W Magnum
10mm Auto
5.45x39mm
5.56x45mm NATO
5.7x28mm
6 ARC
6 Creedmoor
6 PPC
6mm BR Norma
6mm Creedmoor
6.5 Creedmoor
6.5 Grendel
6.5 PRC
6.5 Weatherby RPM
6.5x47 Lapua
6.5x55 Swedish Mauser
6.5x284 Norma
6.8 SPC
6.8 Western
6.8x51mm
7.62x39mm
7.62x51mm NATO
7.62x54mmR
7mm PRC
7mm Remington Magnum
7mm Weatherby Magnum
7mm Win Short Magnum
7mm-08 Remington
8.6 Blackout
8x57 Mauser
9mm Luger
9mm Makarov
`;

export const MAGPUL_GLYPHS_RAW = `# Magpul PMAG Gen M3 dot-matrix glyph font (#20).
#
# Format: one glyph per line —
#   <character> <row1> <row2> <row3> <row4> <row5>
# where each row is exactly 3 characters wide: \`#\` marks a painted dot, \`.\`
# marks an unpainted position. Example shape (illustrative only, not an
# authoritative transcription):
#   4 #.# #.# ### ..# ..#
#
# Source of truth: Magpul's published PMAG Gen M3 dot-matrix diagram —
# https://magpul.com/media/wysiwyg/Instructions/Magpul_Dot_Matrix.pdf
# Cross-check every transcribed row against the community reference below
# before treating it as authoritative; resolve any discrepancy against the
# Magpul PDF, which wins ties —
# https://www.ar15.com/media/viewFile.html?i=36785
#
# This file ships with zero glyph rows until the diagram is transcribed
# (KTD3 of docs/plans/2026-08-02-001-feat-magazine-dot-matrix-label-plan.md).
# An empty table is what makes the feature ship dark: the dot-matrix label
# renders nothing rather than a guessed or wrong pattern. Blank lines and
# lines beginning with \`#\` in column zero are comments and are skipped by
# the parser (src/domain/magazines/glyphs.ts).
#
# Covers R1 (glyph coverage: 0-9, A-Z, hyphen) and R2 (fixed 3x5 dot cell).
`;

export const MANUFACTURERS_RAW = `2A Armament
Accuracy International
Adams Arms
Aero Precision
Alex Pro Firearms
Alexander Arms
Altor Corporation
American Classic
American Defense Manufacturing
American Precision Arms
American Tactical (ATI)
Anderson Manufacturing
Angstadt Arms
Anschutz
ArmaLite
Armscor
Arex Defense
Arsenal Inc.
Atlas Gunworks
B&T AG
Barrett Firearms Manufacturing
Battle Arms Development
BCM (Bravo Company Manufacturing)
Bear Creek Arsenal
Benelli USA
Bergara
Beretta USA
Bersa
Big Horn Armory
Black Aces Tactical
Black Rain Ordnance
Blaser
Bond Arms
Brigade Manufacturing
Browning
Bushmaster Firearms International
Canik
Caracal International
Century Arms International
Charles Daly
Charter Arms
Chiappa Firearms
Christensen Arms
Cimarron Firearms
Citadel
CMMG
Colt
Connecticut Valley Arms (CVA)
Core15 Rifle Systems
CORE Rifle Systems
Crye Precision
CZ-USA
Daniel Defense
Dan Wesson Firearms
Dark Storm Industries
Davidson Defense
Daewoo Precision
Del-Ton
Desert Tech
Diamondback Firearms
DS Arms
EAA Corporation
Ed Brown Products
EMF Company
Emtan
Excel Arms
F-1 Firearms
FabArm USA
FEG
FMK Firearms
FN America
Foxtrot Mike Products
Franklin Armory
Freedom Ordnance Manufacturing
Gibbs Rifle Company
Girsan
Glock
Grand Power
Grey Ghost Precision
H&K (Heckler & Koch)
Hammerli
Heizer Defense
Henry Repeating Arms
Hera Arms
Heritage Manufacturing
Hi-Point Firearms
High Standard Manufacturing
Hudson Manufacturing
Inland Manufacturing
ISSC Austria
IWI US (Israel Weapon Industries)
JTS Group
Just Right Carbines
Kahr Arms
Kalashnikov Concern
Kalashnikov USA
Kel-Tec
Kimber
Knight's Armament Company
KRISS USA
LanTac USA
Lead Star Arms
Les Baer Custom
Lionheart Industries
LMT (Lewis Machine & Tool)
Lone Wolf Arms
LWRC International
Magnum Research
Masterpiece Arms
Maverick Arms
Maxim Defense
Mauser
McMillan Firearms Manufacturing
MDX Arms
Milkor
Molot
Morini
Mossberg
Nighthawk Custom
Noreen Firearms
Norinco
North American Arms
Nosler
Oberland Arms
Olympic Arms
Palmetto State Armory
Para USA
Patriot Ordnance Factory
Pietta
Pindad
Primary Weapons Systems
PTR Industries
Radical Firearms
Remington Arms
Republic Forge
Robinson Armament
Rock Island Armory
Rock River Arms
Rossi
Ruger
Sako
SAR USA
Sarsilmaz
Savage Arms
Schuetzen Gun Works
SCCY Industries
Seekins Precision
Shadow Systems
Shooter's Arms Manufacturing
SIG Sauer
Smith & Wesson
Sphinx Systems
Springfield Armory
Staccato (STI International)
Stag Arms
Standard Manufacturing
Steyr Arms
Steyr AG
Stoeger Industries
Sturm, Ruger & Co.
Swiss Arms AG
Tactical Solutions
Taurus
Taylor's & Company
Tikka
Thompson/Center Arms
TNW Firearms
Traditions Firearms
Tristar Firearms
Troy Industries
Uberti
Umarex USA
Walther Arms
Weatherby
Wilson Combat
Winchester Firearms
Windham Weaponry
Zastava Arms
ZiP Factory
Auto-Ordnance
Cobra Firearms
Colt Canada
Freedom Arms
Howa
Iver Johnson Arms
Marlin Firearms
Pointer
Remington Defense
`;
